...
const TemplateInput = (props) => {
  ...
  return (
    <>
      {fileUrls.length > 0 ? (<div className={`dragdrop-uploaded dragdrop-uploaded-single ${options === 'mini' ? 'dragdrop-uploaded-mini' : ''}`}>
        {renderImgs()}
      </div>) : (<div className={`dragdrop-zone ${options === 'mini' ? 'mini' : ''} ${isDragActive ? 'dragged' : ''}`} {...getRootProps()}>
        <input {...getInputProps()} />
        {
          loadingFile === true ?
            ...
            <>
              ...
              <span>Перетащите файл сюда или нажмите, чтобы выбрать файлы</span>
            </>
        }
      </div>)}
    </>
